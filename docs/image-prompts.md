...
## Sauna-profielen: bewust géén AI-sfeerbeelden

> **Beslissing (26 juli 2026):** voor sauna-profielpagina's genereren we geen
> AI-sfeerbeelden. Een beeld op een locatiepagina wekt de indruk dat de locatie er
> zo uitziet, ook als het een generieke impressie is — misleidend voor bezoekers en
> slecht voor vertrouwen/E-E-A-T. Profielen zonder echte foto tonen het logo van de
> sauna (automatisch via `npm run fetch-logos`); de enige juiste upgrade is écht
> beeldmateriaal van de sauna zelf, met toestemming (bijv. persfoto's opvragen als
> onderdeel van het voor-sauna's-contact). Event-beelden (sfeer, geen locatie)
> blijven wél via de prompts hierboven.
