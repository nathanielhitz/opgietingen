...
**Env / secrets:** `FIRECRAWL_API_KEY` (fetch + primaire extractie + verify-fallback), `ANTHROPIC_API_KEY` (extractie-fallback + nieuwsbrief-extractie), `MAIL_IMAP_*` (nieuwsbrief-inbox), `MAIL_VERTROUWDE_AFZENDERS` (doorstuur-route), `BUFFER_API_KEY` (Buffer-adapter; zonder sleutel slaat die zichzelf over), `BUFFER_KANAAL_*` (optionele kanaal-id-overrides voor de Buffer-adapter). Lokaal via `.env`/export; in CI via GitHub Actions secrets.
