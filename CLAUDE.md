...
Gebruiksregels:
- **Concepts niet verwijderen.** Dedup werkt op `saunaSlug + startDatum`; een verwijderd concept komt de volgende run terug. Afwijzen = status op `afgewezen` (onzichtbaar, valt uit het weekrapport). Let op: `afgelopen` maakt een event níet onzichtbaar — de "afgelopen"-weergave hangt aan de datum.
