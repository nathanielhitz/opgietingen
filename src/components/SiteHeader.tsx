    ...
    <header className="sticky top-0 z-40 border-b border-sand/70 bg-cream/85 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        <Link href="/" className="group flex items-center gap-2.5">
          <span
            aria-hidden
            className="grid h-9 w-9 place-items-center rounded-full bg-wood-dark text-ember-soft shadow-sm transition-transform group-hover:scale-105"
          >
            {/* stoomwolkje-glyph */}
        ...
        <nav className="flex items-center gap-1 text-sm sm:gap-2">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="hidden rounded-full px-3 py-1.5 font-medium text-ink-soft transition-colors hover:bg-sand hover:text-ink sm:inline-block"
            >
              {item.label}
            </Link>
          ))}
