...
src/
  ...
  lib/
    ...
    scraper.ts      # scraper-laag: Firecrawl (markdown + extractie) + Codex-fallback
scripts/
...
> Model: de fallback-extractie gebruikt bewust `Codex-haiku-4-5` (snel/goedkoop). Wijzig via `FALLBACK_MODEL` in `src/lib/scraper.ts`.
